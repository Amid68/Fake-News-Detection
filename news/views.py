...
from .models import Article, Source, Topic, UserSavedArticle, ArticleViewHistory, DetectionModelMetrics
    ...
    if request.user.is_authenticated and not (source_id or topic_slug):
        ...
        if preferences.exists():
            ...
            for pref in preferences:
                ...
                preference_query |= Q(title__icontains=keyword) | Q(content__icontains=keyword)
...
def article_detail_view(request, article_id):
    ...
    related_articles = Article.objects.select_related('source').filter(
        Q(topics__in=article.topics.all()) | Q(source=article.source)
    ).exclude(id=article_id).distinct().order_by('-publication_date')[:5]
...
def save_article_view(request, article_id):
    ...
    if created:
        messages.success(request, f'Article "{article.title[:50]}..." saved for later reading.')
    else:
...
def unsave_article_view(request, article_id):
    ...
    if deleted:
        messages.success(request, f'Article "{article.title[:50]}..." removed from your saved list.')
    else:
...
def saved_articles_view(request):
    ...
    """
    saved_articles = UserSavedArticle.objects.filter(
        user=request.user
...
def search_view(request):
    ...
    if query:
        ...
        articles = Article.objects.select_related('source').filter(
...
def topic_detail_view(request, topic_slug):
    ...
    articles = Article.objects.filter(topics=topic).select_related('source').order_by('-publication_date')
