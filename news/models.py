...
class Source(models.Model):
    ...
    name = models.CharField(max_length=255, unique=True, help_text='Name of the news source')
    base_url = models.TextField(help_text="Base URL of the news source's website")
    api_endpoint = models.TextField(blank=True, null=True, help_text='API endpoint URL, if available')
    description = models.TextField(blank=True, null=True, help_text='Brief description of the news source')
    logo_url = models.URLField(blank=True, null=True, help_text="URL to the source's logo image")
    reliability_score = models.FloatField(blank=True, null=True, help_text='Reliability score (0-10)')
    is_active = models.BooleanField(default=True, help_text='Whether the source is currently active')
...
class Topic(models.Model):
    ...
    slug = models.SlugField(unique=True, help_text='URL-friendly version of the topic name')
    description = models.TextField(blank=True, null=True, help_text='Description of the topic')
...
class Article(models.Model):
    ...
    content = models.TextField(blank=True, null=True, help_text='Full content of the article')
    summary = models.TextField(blank=True, null=True, help_text='AI-generated summary')
    source = models.ForeignKey(Source, on_delete=models.CASCADE, help_text='Source of the article')
    author = models.CharField(max_length=255, blank=True, null=True, help_text='Author of the article')
    publication_date = models.DateTimeField(help_text='When the article was published')
    source_article_url = models.TextField(unique=True, help_text='Original URL of the article')
    featured_image_url = models.URLField(blank=True, null=True, help_text="URL to article's featured image")
    bias_score = models.FloatField(blank=True, null=True, help_text='Political bias score (-1.0 left to 1.0 right)')
    topics = models.ManyToManyField(Topic, blank=True, related_name='articles',
                                    help_text='Topics related to this article')
    view_count = models.PositiveIntegerField(default=0, help_text='Number of times the article has been viewed')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class UserSavedArticle(models.Model):
    ...
    notes = models.TextField(blank=True, null=True, help_text="User's personal notes about the article")
...
class FakeNewsDetectionResult(models.Model):
    ...
    article = models.OneToOneField(Article, on_delete=models.CASCADE, related_name='fake_news_detection')
    ...
    explanation = models.TextField(blank=True, null=True, help_text='Explanation of the detection result')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class DetectionModelMetrics(models.Model):
    ...
    efficiency_score = models.FloatField(help_text='Composite score of performance and resource usage')
    updated_at = models.DateTimeField(auto_now=True)
