...
class ArticleViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated and not (source_id or topic_slug):
            if (
                hasattr(self.request.query_params, "personalized")
                and self.request.query_params.get("personalized") == "true"
            ):
                ...
                if preferences.exists():
                    ...
                    for pref in preferences:
                        topic_query |= Q(content__icontains=pref.topic_keyword) | Q(
                            title__icontains=pref.topic_keyword
                        )
